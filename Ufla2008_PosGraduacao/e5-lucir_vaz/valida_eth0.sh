...
if [ -z "${client[2]}" ] ||  [ -z "${bandwidth[2]}" ] || [ -z "${limit[2]}" ] || [ -z "${priority[2]}" ] || [ -z "${src[2]}" ]; then
...
echo "<TR><TD><H4><I>Classe Principal da Rede Externa e seus Clientes</I></H4></TD></TR>"
...
<TD><input value=\"${src[$c]}\" size="19" maxlength="19" readonly="readonly" name="src[$c]"</TD>
<TD>"${erro[$c]}"</TD>
</TR>"
c=$((c+1))
done
...
#   echo "</form> </body> </html>"
     ...
     if [ -f "$CFG/$ETH" ]; then
       if [ ! -w "$CFG/$ETH" ]; then
         ...
         eval mv -f "$CFG/$ETH" "$CFG/$ETH"_"$INICIO"
